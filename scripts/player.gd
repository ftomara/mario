...
const SMALL_MARIO_COLLISIONSHAPE = preload("res://Resources/CollisionShapes/small_mario_collisionshape.tres")
...
func _on_area_2d_area_entered(area):
	...
	if area is Mashroom:
		handle_mashroom_collision(area)	
		area.queue_free()
	if area is ShootingFlower:
		handle_flower_collision(area)
		
func handle_flower_collision(area):
...
func handle_mashroom_collision(area : Node2D):
...
func set_collision_shapes(is_small : bool):
	var collision_shape = SMALL_MARIO_COLLISIONSHAPE if is_small else BIG_MARIO_COLLISION_SHAPE
...
func switch_to_underground():
	var lvl_man = get_tree().get_first_node_in_group("level_manger")
	SceneData.player_mode = player_mode
	SceneData.coins = lvl_man.coins
	SceneData.points = lvl_man.points
	get_tree().change_scene_to_file("res://scenes/underground.tscn")
func handle_pipe_connector_enterance_collision():
	set_physics_process(false)
	var pipe_tween = get_tree().create_tween()
...
func go_to_castle():
		...
		var run_to_castle = get_tree().create_tween()
		run_to_castle.tween_property(self,"position", position + Vector2(75,0),.5)
		run_to_castle.tween_callback(finish)
